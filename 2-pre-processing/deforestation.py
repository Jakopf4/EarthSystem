...
def deforestation_network_BaU_one_step(scenario: int, year: int, month: int):
    ...
    txt_filepath = f"../1-data/deforestation/deforestation_all_BaU_{year}.txt"
    ...
    ds_new["prec"] = ds_new["prec"] - (orig_inflow - new_inflow)

    # (Optional) Falls auch die 'evap' Variable lokal reduziert werden soll:
    ds_new["evap"] = ds["evap"] * ratios
...
def deforestation_network_Gov_one_step(scenario: int, year: int, month: int):
    ...
    """
    nc_filepath = f"../1-data/water/scenario_ssp{scenario}_decade{year}_month{month:02d}.nc"
    txt_filepath = f"../1-data/deforestation/deforestation_all_Gov_{year}.txt"
...
def deforestation_network_Sec(scenario: int, year: int, month: int):
    ...
    """
    nc_filepath = f"../1-data/water/scenario_ssp{scenario}_decade{year}_month{month:02d}.nc"
    sec_filepath = f"../1-data/water_secondary_vegetation/scenario_ssp{scenario}_decade{year}_month{month:02d}.nc"
    ...
    # Logik: Wie viel Wasser kommt flussabwärts bei jeder Senke (y) an? -> Spaltensummen (axis=0)
    orig_inflow = np.sum(orig_matrix, axis=0)
    new_inflow = np.sum(sec_matrix, axis=0)

    # Verhältnis = Neu / Alt. 'where=orig_inflow!=0' verhindert Division durch 0.
    ratios_in = np.divide(new_inflow, orig_inflow, out=np.ones_like(orig_inflow), where=(orig_inflow != 0))

    # Originalen Niederschlag prozentual skalieren
    ds_sec["prec"] = ds_orig["prec"] * ratios_in

    # --- B. VERDUNSTUNG (evap) anpassen ---
    # Logik: Wie viel Wasser gibt eine Quelle (x) an das Netzwerk ab? -> Zeilensummen (axis=1)
    orig_outflow = np.sum(orig_matrix, axis=1)
    new_outflow = np.sum(sec_matrix, axis=1)

    # Verhältnis = Neu / Alt. 
    ratios_out = np.divide(new_outflow, orig_outflow, out=np.ones_like(orig_outflow), where=(orig_outflow != 0))

    # Originale Verdunstung prozentual skalieren
    ds_sec["evap"] = ds_orig["evap"] * ratios_out

    # --- C. DATENSATZ SPEICHERN ---
    ...
    ds_orig.close()
    ds_sec.close()
    # print(f"Sekundärvegetation (Sec) für Jahr {year}, Monat {month} erfolgreich aktualisiert und unter '{output_filepath}' gespeichert.")
...
def deforestation_network_BaU_two_step(scenario: int, year: int, month: int):
    ...
    """
    nc_filepath = (
        f"../1-data/water/scenario_ssp{scenario}_decade{year}_month{month:02d}.nc"
    )
    txt_filepath = f"../1-data/deforestation/deforestation_all_BaU_{year}.txt"
